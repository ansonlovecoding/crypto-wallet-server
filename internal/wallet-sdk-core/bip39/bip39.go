...
import (
	"Share-Wallet/internal/wallet-sdk-core/bip39/wordlists"
	"crypto/rand"
	"crypto/sha256"
	"crypto/sha512"
	"encoding/binary"
...
var (
	...
	wordListLangMap = map[string][]string{
		EN:    wordlists.English,
		CHSim: wordlists.ChineseSimplified,
		CHTra: wordlists.ChineseTraditional,
		FR:    wordlists.French,
		IT:    wordlists.Italian,
		JA:    wordlists.Japanese,
		KO:    wordlists.Korean,
		SP:    wordlists.Spanish,
	}
)
...
func SetWordListLanguage(lang string) {
	if list, ok := wordListLangMap[lang]; ok {
		SetWordList(list)
	} else {
		SetWordList(wordlists.English)
	}
