...
func (b *Bitcoin) GetFee(tx *wire.MsgTx, adjustmentFee float64) (btcutil.Amount, error) {
	...
	if err != nil {
		log.Warn("fail to call btc.getMinRelayFee() but continue", zap.Error(err))
	} else {
		if fee < relayFee {
			fee = relayFee
		}
	...
	if b.validateAdjustmentFee(adjustmentFee) {
		...
		if err != nil {
			log.Warn("fail to call btc.calculateNewFee() but continue", zap.Error(err))
		}
		log.Debug("called btc.calculateNewFee()", zap.Any("adjusted newFee", newFee)) // 0.000208 BTC
		fee = newFee
	}
